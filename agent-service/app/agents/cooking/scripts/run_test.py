...
def L(name, unit, qty="500"):  # noqa: N802 — compact inventory-lot factory for the debug harness
    return InventoryLotSnapshot(lot_id=f"l_{name}", item_id=f"i_{name}", canonical_name=name, on_hand=Decimal(qty), reserved=Decimal(0), unit=unit)

INVENTORY = (
    L("蟹脚", "g", "2000"), L("鲜虾", "g", "1000"), L("基围虾", "g", "800"),
    L("排骨", "g", "1000"), L("鸡翅中", "个", "30"), L("腊肠", "根", "4"),
    L("包菜", "个", "2"), L("菜花", "个", "2"), L("玉米", "根", "3"),
    L("胡萝卜", "根", "3"), L("蒜苗", "根", "5"), L("洋葱", "个", "2"),
    L("姜", "g"), L("生姜", "g"), L("蒜", "g", "500"), L("大蒜", "g"),
    L("蒜头", "g"), L("蒜末", "g"), L("葱", "g", "300"), L("葱段", "g"),
    L("葱花", "g"), L("干辣椒", "g", "200"), L("辣椒", "g"),
    L("小米辣", "g"), L("青花椒", "g"), L("红枣", "个", "20"),
    L("盐", "g", "1000"), L("生抽", "ml"), L("老抽", "ml"), L("蚝油", "ml"),
    L("料酒", "ml"), L("米酒", "ml"), L("香醋", "ml"), L("白糖", "g"),
    L("豆瓣酱", "g"), L("火锅底料", "g", "300"), L("火锅料", "g", "300"),
    L("辣椒面", "g"), L("胡椒粉", "g"), L("白胡椒粉", "g"),
    L("黑胡椒粉", "g"), L("鸡精", "g"), L("味精", "g"), L("鸡粉", "g"),
    L("酒糟", "g"), L("白芝麻", "g"), L("五香粉", "g"),
    L("食用油", "ml", "2000"), L("猪油", "g"), L("淀粉", "g"),
    L("蛋清", "个", "10"), L("水淀粉", "ml"),
)

RESOURCES = (
    KitchenResourceSnapshot(resource_id="r1", resource_type="stove", capacity=Decimal(2), capacity_unit="burners", capabilities=("gas",), available=True),
    KitchenResourceSnapshot(resource_id="r2", resource_type="stove", capacity=Decimal(2), capacity_unit="burners", capabilities=("gas",), available=True),
    KitchenResourceSnapshot(resource_id="r3", resource_type="oven", capacity=Decimal(1), capacity_unit="racks", capabilities=("convection",), available=True),
    KitchenResourceSnapshot(resource_id="r4", resource_type="sink", capacity=Decimal(2), capacity_unit="basins", capabilities=(), available=True),
    KitchenResourceSnapshot(resource_id="r5", resource_type="rice_cooker", capacity=Decimal(1), capacity_unit="pots", capabilities=("stew",), available=True),
    KitchenResourceSnapshot(resource_id="r6", resource_type="knife", capacity=Decimal(3), capacity_unit="pieces", capabilities=("cutting", "chopping"), available=True),
    KitchenResourceSnapshot(resource_id="r7", resource_type="pot", capacity=Decimal(4), capacity_unit="pieces", capabilities=("boiling", "stewing", "stir_frying"), available=True),
    KitchenResourceSnapshot(resource_id="r8", resource_type="cutting_board", capacity=Decimal(2), capacity_unit="pieces", capabilities=("cutting",), available=True),
    KitchenResourceSnapshot(resource_id="r9", resource_type="wok", capacity=Decimal(2), capacity_unit="pieces", capabilities=("stir_frying", "deep_frying"), available=True),
    KitchenResourceSnapshot(resource_id="r10", resource_type="spatula", capacity=Decimal(3), capacity_unit="pieces", capabilities=("stirring", "flipping"), available=True),
    KitchenResourceSnapshot(resource_id="r11", resource_type="mixing_bowl", capacity=Decimal(3), capacity_unit="pieces", capabilities=("mixing", "marinating"), available=True),
)


async def main():
    request = GeneratePlanRequest(
        request_id="t", user_id="u",
        recipes=RECIPES, inventory_lots=INVENTORY, kitchen_resources=RESOURCES,
    )
