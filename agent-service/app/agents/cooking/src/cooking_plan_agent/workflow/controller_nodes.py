# =============================================================================
# ReAct 控制器节点（workflow/controller_nodes）
# -----------------------------------------------------------------------------
# 控制器循环：agent_controller（LLM 决策）→ run_tool（执行）→ 观察回填 → 回控制器。
# 两条硬保障：步数耗尽强制回退确定性 DAG；控制器缺失 / 失败 / 非法决策降级为
# agent_mode="deterministic"，绝不静默放行。本模块节点永不抛异常、永不写 WorkflowError。
# =============================================================================
