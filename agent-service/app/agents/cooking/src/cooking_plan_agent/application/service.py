...
class GenerateCookingPlanService:
    ...
    async def preprocess(
        ...
        Reuses the same parsing and local-inference modules as the generate
        workflow (``RecipeExtractor``, ``find_recipe_gaps``, ``infer_local``,
        ``merge_inference``), so the agent owns a single implementation of
        recipe understanding. The backend calls this endpoint BEFORE
        generate() and passes the returned candidates back as
        ``preparsed_candidates`` — generate then never re-parses and never
        asks gap/assumption questions.

        Args:
            body: Recipes as raw text (NL parsing + gap filling source).

        Returns:
            PreprocessRecipesResponse with one fully-populated candidate per
            input recipe — missing durations/heat/temperature/resources are
            inferred via local rules.
        """
        import asyncio
        ...
        # These recipes have already crossed the recipe-import boundary and
        # are stored by the backend as normalized fields. Re-running the LLM
        # here adds a provider round trip (and its retry budget) to every plan
        # generation while providing no new user input. Free-form multi-dish
        # parsing remains owned by ParseRecipeImportService; this boundary is
        # intentionally deterministic and local.
        extractor = RuleExtractor()

        def _fill_gap(candidate, gap):
            """Fill a gap with local rules regardless of gap class.

            Unlike ``infer_local`` (which only fills critical/safety-critical
            gaps), the preprocess endpoint fills every gap it can — the
            backend needs a complete candidate so generate() never re-asks
            gap or assumption questions.
            """
            from cooking_plan_agent.parsing.inference import _detect_primary_technique
        ...
        async def _process_one(recipe) -> ExtractedRecipeCandidate:
            ...
            from cooking_plan_agent.parsing.inference import InferenceResult

            merged = merge_inference(candidate, InferenceResult(filled_gaps=tuple(filled), unresolved_gaps=(), assumptions=()))
            # P0-04 rule 2: the caller's recipe_id overrides the extractor's
            # internal ID so the returned snapshot is addressable by the same
            # key the generate request will use.
            return merged.model_copy(update={"recipe_id": recipe.recipe_id})

        candidates = await asyncio.gather(*(_process_one(r) for r in body.recipes))
