...
class TestConvertCrossDimension:
    ...
    def test_piece_to_g_with_conversion(self, converter: UnitConverter, onion_conversion: ProductConversion) -> None:
        result = converter.convert(Decimal(3), "piece", "g", onion_conversion)
        assert result == Decimal(450)  # 3 × 150

    def test_fractional_piece(self, converter: UnitConverter, onion_conversion: ProductConversion) -> None:
    ...
    def test_conversion_unit_mismatch_from(self, converter: UnitConverter, onion_conversion: ProductConversion) -> None:
        with pytest.raises(UnitConversionError, match="from_unit"):
            converter.convert(Decimal(2), "pc", "g", onion_conversion)

    def test_conversion_unit_mismatch_to(self, converter: UnitConverter, onion_conversion: ProductConversion) -> None:
...
class TestScaleIngredient:
    ...
    def test_scale_up(self, converter: UnitConverter, sample_demand: IngredientDemand) -> None:
    ...
    def test_scale_down(self, converter: UnitConverter, sample_demand: IngredientDemand) -> None:
    ...
    def test_identity(self, converter: UnitConverter, sample_demand: IngredientDemand) -> None:
    ...
    def test_fractional_servings(self, converter: UnitConverter, sample_demand: IngredientDemand) -> None:
        ...
        assert result.quantity.quantize(Decimal("0.0001")) == expected.quantize(Decimal("0.0001"))

    def test_does_not_mutate_input(self, converter: UnitConverter, sample_demand: IngredientDemand) -> None:
    ...
    def test_different_fields_preserved(self, converter: UnitConverter, sample_demand: IngredientDemand) -> None:
    ...
    def test_original_servings_zero_raises(self, converter: UnitConverter, sample_demand: IngredientDemand) -> None:
        with pytest.raises(InvalidQuantityError, match="original_servings"):
            scale_ingredient(sample_demand, Decimal(0), Decimal(4))

    def test_original_servings_negative_raises(self, converter: UnitConverter, sample_demand: IngredientDemand) -> None:
        with pytest.raises(InvalidQuantityError, match="original_servings"):
            scale_ingredient(sample_demand, Decimal(-1), Decimal(4))

    def test_target_servings_zero_raises(self, converter: UnitConverter, sample_demand: IngredientDemand) -> None:
        with pytest.raises(InvalidQuantityError, match="target_servings"):
            scale_ingredient(sample_demand, Decimal(4), Decimal(0))

    def test_target_servings_negative_raises(self, converter: UnitConverter, sample_demand: IngredientDemand) -> None:
        with pytest.raises(InvalidQuantityError, match="target_servings"):
