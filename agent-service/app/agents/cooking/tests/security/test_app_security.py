...
class TestPromptInjection:
    ...
    def test_xml_instruction_injection_treated_as_data(self) -> None:
        """XML/JSON instruction injection must be stripped as markup, not executed."""
...
# =============================================================================
# 5. Search-result prompt injection → sanitised
# =============================================================================


class TestSearchResultInjection:
    def test_injection_in_search_snippet_detected(self) -> None:
        """Prompt injection in search result snippets must be detected."""
        from cooking_plan_agent.research.text_sanitizer import sanitize_document_content

        injected = "<script>alert('hack')</script>Cook chicken for 10 minutes."
        cleaned, has_injection = sanitize_document_content(injected)
        assert has_injection or "script" not in cleaned.lower()

    def test_html_tags_stripped_from_search_results(self) -> None:
        """HTML markup in search results must be stripped."""
        from cooking_plan_agent.research.text_sanitizer import strip_markup

        text = "<p>Cook chicken for <b>10 minutes</b> on <em>high</em> heat.</p>"
...
# =============================================================================
# 6. SSRF prevention — no arbitrary URL fetch
# =============================================================================


class TestSSRFPrevention:
    def test_domain_allowlist_blocks_arbitrary_urls(self) -> None:
        """Only allow-listed domains pass filtering."""
        ...
        # localhost must be dropped
        assert all(d.domain != "localhost" for d in filtered)
        assert len(filtered) == 1

    def test_private_ip_blocked(self) -> None:
        """Private/loopback IPs must be blocked by domain filter."""
        from cooking_plan_agent.domain.models import SearchDocument
        from cooking_plan_agent.research.config import DomainAllowList
        from cooking_plan_agent.research.domain_filter import filter_by_domain
