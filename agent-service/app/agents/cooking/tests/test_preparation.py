...
class TestDecomposeSimmer:
    ...
    def test_simmer_with_remainder(self, policy: DecompositionPolicy) -> None:
        ...
        passive_total = sum(
            t.duration_minutes for t in tasks if t.work_mode == WorkMode.PASSIVE
        )
...
class TestInsertOperationChain:
    ...
    def test_root_tracks_demand_ids(self) -> None:
        root = PrepTrieNode(operation_key="root")
        chain = (
            PreparationOperation(operation="wash", quantity=Decimal(50), unit="g"),
        )
        insert_operation_chain(root, chain, "d1")
...
class TestConvertTrieToTasks:
    ...
    def test_produces_flat_task_list(self, populated_trie: PrepTrieNode) -> None:
        tasks = convert_trie_to_tasks(
            populated_trie, "chilli", ("dish-a", "dish-b")
        )
    ...
    def test_wash_task_has_no_dependency(self, populated_trie: PrepTrieNode) -> None:
        ...
        wash_task = next(
            t for t in tasks if t.task_id.startswith("prep_chilli_wash")
        )
...
class TestTaskGraph:
    ...
    def test_build_graph_deduplicates_edges(self) -> None:
        ...
        t2 = CookingTask(
            ...
            dependencies=(
                TaskDependency(predecessor_id="t1"),
            ),
        )
        graph = build_task_graph((t1, t2), (), ())
...
class TestTopologicalSort:
    ...
    def linear_graph(self) -> TaskGraph:
        """t1 → t2 → t3 (no cycles)."""
        t1 = CookingTask(
            task_id="t1", dish_id="r1", instruction="A",
            duration_minutes=1, work_mode=WorkMode.ACTIVE, category="test",
        )
        t2 = CookingTask(
            task_id="t2", dish_id="r1", instruction="B",
            duration_minutes=1, work_mode=WorkMode.ACTIVE, category="test",
        )
        t3 = CookingTask(
            task_id="t3", dish_id="r1", instruction="C",
            duration_minutes=1, work_mode=WorkMode.ACTIVE, category="test",
        )
        edges = (
    ...
    def test_single_task(self) -> None:
        t = CookingTask(
            task_id="alone", dish_id="r1", instruction="X",
            duration_minutes=1, work_mode=WorkMode.ACTIVE, category="test",
        )
        graph = TaskGraph(tasks=(t,), edges=())
        result = topological_sort_kahn(graph)
    ...
    def test_diamond_dag(self) -> None:
        ...
        tasks = tuple(
            CookingTask(
                task_id=tid, dish_id="r1", instruction=tid,
                duration_minutes=1, work_mode=WorkMode.ACTIVE, category="test",
            )
...
class TestCriticalPath:
    def test_linear_chain(self) -> None:
        ...
        t1 = CookingTask(
            task_id="t1", dish_id="r1", instruction="A",
            duration_minutes=5, work_mode=WorkMode.ACTIVE, category="test",
        )
        t2 = CookingTask(
            task_id="t2", dish_id="r1", instruction="B",
            duration_minutes=10, work_mode=WorkMode.ACTIVE, category="test",
        )
        t3 = CookingTask(
            task_id="t3", dish_id="r1", instruction="C",
            duration_minutes=3, work_mode=WorkMode.ACTIVE, category="test",
        )
        edges = (
            TaskEdge(predecessor_id="t1", successor_id="t2"),
            TaskEdge(predecessor_id="t2", successor_id="t3"),
...
class TestIntegration:
    ...
    def test_full_pipeline(self) -> None:
        ...
        updated_first = first_stir.model_copy(
            update={
                "dependencies": first_stir.dependencies
                + (TaskDependency(predecessor_id=last_boil.task_id),),
            }
        )
        recipe_tasks = (
            recipe_tasks[:3] + (updated_first,) + recipe_tasks[4:]
        )
    ...
    def test_preparation_and_recipe_integration(self) -> None:
        ...
        chain = (
            PreparationOperation(operation="wash", quantity=Decimal(100), unit="g"),
            PreparationOperation(
                operation="dice", specification="diced", quantity=Decimal(100), unit="g"
            ),
        )
        insert_operation_chain(root, chain, "d1")
